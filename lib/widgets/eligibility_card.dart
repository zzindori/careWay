...
class EligibilityCard extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (profile.hasLtcGrade && ltc != null) ...[
          ...
          _buildRow('장기요양 서비스', EligibilityResult.eligible('등급 보유 - 서비스 이용 가능')),
          const SizedBox(height: 8),
        ],

        // 건강검진
        _buildRow('건강검진 (${DateTime.now().year}년)', checkup),

        // 기초수급자 추가
        if (profile.isBasicRecipient) ...[
          const SizedBox(height: 8),
          _buildRow('기초생활수급', EligibilityResult.eligible('수급자 전용 복지 추가 혜택 있음')),
        ],

        // 독거 추가
        if (profile.liveAlone) ...[
          const SizedBox(height: 8),
          _buildRow('독거노인 서비스', EligibilityResult.eligible('노인맞춤돌봄, 응급안전안심 등 대상')),
        ],
      ]),
    );
  }

  Widget _buildRow(String label, EligibilityResult result) {
  ...
  Widget _buildLtcRow(ltc) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
      decoration: BoxDecoration(color: AppTheme.primary.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(8)),
      child: Row(children: [
