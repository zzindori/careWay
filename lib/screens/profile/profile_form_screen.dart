...
class _ProfileFormScreenState extends State<ProfileFormScreen> {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      DropdownButtonFormField<String>(
        ...
        onChanged: (v) => setState(() {
          _region = v ?? _region;
          final subs = _subRegionsByRegion[_region] ?? const <String>[];
          if (!subs.contains(_subRegion)) {
            _subRegion = '';
          }
        }),
      ),
      ...
      DropdownButtonFormField<String>(
        initialValue: (_subRegionsByRegion[_region] ?? const <String>[]).contains(_subRegion)
            ? _subRegion
            : null,
        decoration: const InputDecoration(hintText: '시/군/구 선택'),
        items: [
          ...(_subRegionsByRegion[_region] ?? const <String>[])
              .map((s) => DropdownMenuItem(value: s, child: Text(s))),
          if (_subRegion.isNotEmpty &&
              !(_subRegionsByRegion[_region] ?? const <String>[]).contains(_subRegion))
            DropdownMenuItem(value: _subRegion, child: Text(_subRegion)),
        ],
        onChanged: (v) => setState(() => _subRegion = v ?? ''),
        validator: (v) => (v == null || v.isEmpty) ? '시/군/구를 선택하세요' : null,
      ),
    ]);
  }
  ...
  Widget _buildGenderSelector() {
    const options = [
      ('male', '남성', Icons.male, Colors.blue),
      ('female', '여성', Icons.female, Colors.pink),
      ('unknown', '모름', Icons.help_outline, Colors.grey),
    ];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildLabelWithHelp(
          '성별',
          '성별 정보',
          '일부 복지 서비스는 성별 조건(남성/여성 전용)이 있습니다.\n'
          '정확한 추천을 위해 선택해 주세요.\n'
          '확실하지 않으면 "모름"을 선택하시면 됩니다.',
        ),
        const SizedBox(height: 8),
        Row(
              ...
              child: Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: isSelected ? opt.$4.withValues(alpha: 0.1) : Colors.transparent,
                      border: Border.all(
                        color: isSelected ? opt.$4 : AppTheme.divider,
                    ...
                    child: Column(
                      children: [
                        Icon(opt.$3, size: 18, color: isSelected ? opt.$4 : AppTheme.textSecondary),
                        const SizedBox(height: 4),
                        Text(
                          opt.$2,
                          style: TextStyle(
                            fontSize: 12,
                            color: isSelected ? opt.$4 : AppTheme.textSecondary,
